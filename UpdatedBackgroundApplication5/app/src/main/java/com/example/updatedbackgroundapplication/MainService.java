...
public class MainService extends Service {
    ...
    public static final String apiUrl = "http://192.168.1.104:5000/getjobs/hardware";
    public String unParsed = "";
    public String unParsed2 = "";
    public static final int JOB_ID = 123;
    public static final int JOB_ID_STAT = 124;
            ...
            if(isWifi)
            {
                try {
                    ...
                    if(object.get("jobType")=="PING")
                    {
                        ...
                        { Log.d("Job", "Job ping succsessfully scheduled."); }
                        else { Log.d("Job error", "Job ping scheduling failed.");}
                    }
    ...
    class backgroundTask2 extends TimerTask
    {
        ...
        public void run() {
            ...
            if(isWifi)
            {
                try
                {
                    ...
                    BufferedReader reader = new BufferedReader(new InputStreamReader(stream));
                    String line = "";
                    Log.d("UH2", "Succsessfully made second connection to API.");
                    while(line!=null)
                    {
                        line = reader.readLine();
                        unParsed2 = unParsed2 + line;
                    }
                    JSONArray array = new JSONArray(unParsed2);
                    JSONObject object = (JSONObject) array.get(1);
                    String singleParsed="";
                    singleParsed = singleParsed + object.get("date") + "|" + object.get("jobPeriod") + "|" + object.get("jobType");
                    Log.d("UHM2", singleParsed);
                    if(object.get("jobType")=="STAT")
                    {
                        PersistableBundle bundle2 = new PersistableBundle();
                        bundle2.putString("Data2", singleParsed);
                        ComponentName compName = new ComponentName(getApplicationContext(), jobStat.class);
                        JobInfo info = new JobInfo.Builder(JOB_ID_STAT, compName).setRequiredNetworkType(JobInfo.NETWORK_TYPE_UNMETERED)
                                .setPersisted(true)
                                .setExtras(bundle2)
                                .setOverrideDeadline(4000).build();
                        JobScheduler scheduler = (JobScheduler)getSystemService(JOB_SCHEDULER_SERVICE);
                        int resultCode = scheduler.schedule(info);
                        if(resultCode == JobScheduler.RESULT_SUCCESS)
                        { Log.d("Job2", "Job stat succsessfully scheduled."); }
                        else { Log.d("Job error2", "Job stat scheduling failed.");}
                    }
                    else {Log.d("Job type error2", "Error undefined job type.");}
                } catch (IOException e) {
                    Log.d("Reader error2", "Error: " + e.getMessage());
                } catch (JSONException e) {
                    Log.d("JSON error2", "Error: " + e.getMessage());
                }
            }
            else
            {
               Log.d("Network error2", "Error must be connected to wifi.");
            }
        }
    }
    ...
    public void startTimer()
    {
        ...
        timer2.scheduleAtFixedRate(new backgroundTask2(), 2000, interval);
    }
